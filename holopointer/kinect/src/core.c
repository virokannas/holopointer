...
freenect_context *freenect_allocate_context() {
    freenect_context *ctx = (freenect_context *)malloc(sizeof(freenect_context));
    memset(ctx, 0, sizeof(freenect_context));
    return ctx;
}

freenect_context *freenect_init_bridged(freenect_usb_context *usb_ctx)
{
    freenect_context *ctx = freenect_allocate_context();
    ctx->log_level = LL_WARNING;
    fnusb_init(&(ctx->usb), usb_ctx);
    return ctx;
}
...
freenect_device *freenect_open_device_bridged(freenect_context *ctx, int index) {
    freenect_device *dev;
